...
local Loaded1,Loaded2,PromptLib = false,false,loadstring(game:HttpGet("https://raw.githubusercontent.com/Role34/RobloxRage/main/Utilities/PromptLibrary.lua"))()
...
Notify.Event:Connect(function(Text)
    RobloxRage.Utilities.UI:Notification2(Text)
end)
...
local Window = RobloxRage.Utilities.UI:Window({
    ...
    local VisualsTab = Window:Tab({Name = "Visuals"}) do
        local GlobalSection = VisualsTab:Section({Name = "Global",Side = "Left"}) do
            ...
            --[[GlobalSection:Slider({Name = "Refresh Rate",Flag = "ESP/Player/RefreshRate",Min = 0,Max = 1,Precise = 2,Value = 0.01,
            Callback = function(Value) RobloxRage.Utilities.Drawing.RefreshRate = Value end})]]
        end
        ...
        local MiscSection = MiscTab:Section({Name = "Other",Side = "Left"}) do
            MiscSection:Button({Name = "Redeem Codes",Callback = function()
                ...
                if Success then
                    ...
                    RobloxRage.Utilities.UI:Notification({Title = "RobloxRage",Description = "All available codes are claimed!",Duration = 5})
                else
                    RobloxRage.Utilities.UI:Notification({Title = "RobloxRage",Description = "Failed to get the codes:\n" .. Error,Duration = 5})
                end
            end})
        end
...
local function GetClosest(Enabled,VisibilityCheck,DistanceCheck,
    ...
    for Index,Player in ipairs(PlayerService:GetPlayers()) do
        ...
        for Index,BodyPart in ipairs(BodyParts) do
            ...
            BodyPartPosition = PredictionEnabled and RobloxRage.Utilities.Physics.SolveTrajectory(CameraPosition,BodyPartPosition,
            ...
            if Priority == "Random" then
                ...
                BodyPartPosition = PredictionEnabled and RobloxRage.Utilities.Physics.SolveTrajectory(Camera.CFrame.Position,
...
RobloxRage.Utilities.FixUpValue(Tortoiseshell.Network.Fire,function(Old,Self,...)
    local Args = {...}
    ...
    elseif Args[2] == "__Hit" then
        if SilentAim and not Window.Flags["BB/Rage/Autoshoot/Enabled"] then return end
        if Window.Flags["BB/BulletTracer/Enabled"] then
    ...
    elseif Args[2] == "State" then
        ...
        if Window.Flags["BB/Rage/Autoshoot/Enabled"]
        and table.find(DisabledStates,Args[3]) then Args[4] = false end
...
RobloxRage.Utilities.FixUpValue(Tortoiseshell.Projectiles.InitProjectile,function(Old,Self,...)
    local Args = {...}
...
RobloxRage.Utilities.FixUpValue(Tortoiseshell.Raycast.CastGeometryAndEnemies,function(Old,Self,...)
...
RobloxRage.Utilities.FixUpValue(Tortoiseshell.Items.GetAnimator,function(Old,Self,...)
    local Args = {...}
...
--[[RobloxRage.Utilities.FixUpValue(Tortoiseshell.Items.GetConfig,function(Old,Self,...)
...
HeartbeatConnections["Control"] = function(...)
    ...
    if LPCharacter and LPCharacter.Parent and LPCharacter.PrimaryPart then
        ...
        if Window.Flags["BB/Fly/Enabled"] and FlyPosition then
            FlyPosition += RobloxRage.Utilities.MovementToDirection() * Window.Flags["BB/Fly/Speed"]
...
RobloxRage.Utilities.NewThreadLoop(0.25,function()
    local Weapon,Config = GetEquippedWeapon()
    if Weapon and Config then
...
RobloxRage.Utilities.NewThreadLoop(1/10,function()
    CustomizeWeapon(
