...
local Window = RobloxRage.Utilities.UI:Window({
            ...
            TargetSection:Button({Name = "Teleport",Callback = function()
                ...
                while task.wait() do
                    if not Teleport(PlayerDropdown.Value[1]) then
                        RobloxRage.Utilities.UI:Notification2({Title = "Teleport Ended",Duration = 5})
    ...
    end RobloxRage.Utilities:SettingsSection(Window,"Period",true)
...
local function GetClosest(Enabled,
    ...
    for Index,Player in ipairs(PlayerService:GetPlayers()) do
        ...
        for Index,BodyPart in ipairs(BodyParts) do
            ...
            BodyPartPosition = PredictionEnabled and RobloxRage.Utilities.Physics.SolveTrajectory(Muzzle.Position,
...
local function PlayerFly()
    ...
    RootPart.AssemblyLinearVelocity = Vector3.zero
    RootPart.CFrame += RobloxRage.Utilities.MovementToDirection() * Window.Flags["AR2/Fly/Speed"]
end
local function PlayerWalkSpeed()
    if not PlayerClass.Character then return end
    local RootPart = PlayerClass.Character.RootPart
    local MoveDirection = RobloxRage.Utilities.MovementToDirection() * Vector3.new(1,0,1)
...
RobloxRage.Utilities.FixUpValue(Network.Send,function(Old,Self,Name,...) local Args = {...}
...
--[[RobloxRage.Utilities.FixUpValue(Network.Bounce,function(Old,Self,Name,...) local Args = {...}
    print(Name)
    return Old(Self,Name,unpack(Args))
end)
RobloxRage.Utilities.FixUpValue(Network.Fetch,function(Old,Self,Name,...) local Args = {...}
    print(Name)
    return Old(Self,Name,unpack(Args))
end)]]
...
Events["Inventory Container Added"] = function(Id,Data,...)
    ...
    if Data.WorldPosition and Length(Data.Occupants) > 0 and not string.find(Data.Type,"Corpse") then
        RobloxRage.Utilities.Drawing:AddObject(Data.Id,CIIC(Data),Data.WorldPosition,
...
Events["Container Changed"] = function(Data,...)
    ...
    RobloxRage.Utilities.Drawing:RemoveObject(Data.Id)
    if Data.WorldPosition and Length(Data.Occupants) > 0 and not string.find(Data.Type,"Corpse") then
        RobloxRage.Utilities.Drawing:AddObject(Data.Id,CIIC(Data),Data.WorldPosition,
        "AR2/ESP/Items","AR2/ESP/Items/Containers",Window.Flags)
...
RobloxRage.Utilities.NewThreadLoop(0.1,function()
    local Closest = GetCharactersInRadius(Zombies.Mobs,250)
    if not Closest then return end
...
RobloxRage.Utilities.NewThreadLoop(1,function()
...
for Index,Zombie in pairs(Zombies.Mobs:GetChildren()) do
    ...
    for Index,Inherit in pairs(Config.Inherits) do
        if table.find(ZombieInherits,Inherit) then
            ...
            RobloxRage.Utilities.Drawing:AddObject(
...
for Index,Item in pairs(Loot:GetDescendants()) do
    if Item:IsA("CFrameValue") then
        ...
        RobloxRage.Utilities.Drawing:AddObject(Item,Item.Name,Item.Value.Position,
...
for Index,Event in pairs(Randoms:GetChildren()) do
    if table.find(RandomEvents,Event.Name) then --print(Event.Name)
        RobloxRage.Utilities.Drawing:AddObject(Event,Event.Name,Event.Value.Position,
...
Randoms.ChildAdded:Connect(function(Event)
    if table.find(RandomEvents,Event.Name) then --print(Event.Name)
        ...
        and Window.Flags["AR2/ESP/RandomEvents/" .. Event.Name] then
            ...
            RobloxRage.Utilities.UI:Notification2({Title = Title,Duration = 20})
        end
    end
end)

Zombies.Mobs.ChildRemoved:Connect(function(Zombie)
    RobloxRage.Utilities.Drawing:RemoveObject(Zombie)
end)
Vehicles.ChildRemoved:Connect(function(Vehicle)
    RobloxRage.Utilities.Drawing:RemoveObject(Vehicle)
end)
Loot.DescendantRemoving:Connect(function(Item)
    RobloxRage.Utilities.Drawing:RemoveObject(Item)
end)
Randoms.ChildRemoved:Connect(function(Event)
    RobloxRage.Utilities.Drawing:RemoveObject(Event)
end)
